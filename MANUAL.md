...
- [6. `default-editor`：布局与打包](#6-default-editor布局与打包)
  - [6.1 layout（几何，耦合点 ①）](#61-layout几何耦合点--)
  - [6.2 panel（通用窗格协议）](#62-panel通用窗格协议)
  - [6.3 buffer（打开的文档注册表）](#63-buffer打开的文档注册表)
  - [6.4 frontend（主编辑窗格）](#64-frontend主编辑窗格)
  - [6.5 tree（文件树窗格）](#65-tree文件树窗格)
  - [6.6 status（状态栏窗格）](#66-status状态栏窗格)
  - [6.7 shell（装配，耦合点 ②）](#67-shell装配耦合点--)
  - [6.8 terminal（默认终端后端）](#68-terminal默认终端后端)
...
`screen->string` 是上面「只画文档文本」的简化版（测试 / 无前端驱动用）。完整示例见
`io/example.rkt` 的 `frame->bytes`（`racket io/example.rkt` 即可跑）。
...
core 只管一个 document 的一个 viewport；**窗口管理**在 `default-editor/`（L6），
只依赖 `core/api.rkt` + `core/editor.rkt`。设计目标：各部件（前端 / 文件树 / 状态栏）
彼此独立，**唯一的耦合点是 layout（几何）与命令（输入）**。

```
default-editor/
├── layout.rkt    纯几何：rows×cols → {pane-id → rect} + 显隐         （耦合点 ①）
├── panel.rkt     通用窗格协议：id + 状态 + 投影/尺寸/刷新回调
...
- **组件不互相认识**：`frontend` / `tree` / `status` 只依赖 core + `panel.rkt`，
  不 `require` 彼此。谁在哪个矩形、谁能收键盘，全由 shell 通过 layout + 命令决定。
- **窗口显隐 ≠ 文档开关**：`layout` 与 `panel.visible?` 只管看不看得见；
  document 的 open / close 由 buffer（前端文档）或组件的 `*-close` **显式**做。
  关侧栏不会关文件树文档，隐藏前端也不会关任何打开的文档。
- **编辑后只做廉价同步**：`shell-sync` 只重算状态栏；只有 `shell-refresh`
  （几何变化 / 显式）才全量刷新（文件树重扫 fs）。旧实现每次按键都重扫目录，这里已拆开。
...
| 名字 | 签名 | 语义 |
|---|---|---|
| `layout-open` | `(layout-open rows cols #:sidebar? #t #:sidebar-width 30 #:status? #t #:status-height 1)` | 造布局（自动 normalize） |
| `layout-normalize` | `(layout-normalize l)` | 夹紧：frontend ≥ 1×1；status ≤ rows-1；tree ≤ cols-1 |
| `layout-frontend` / `layout-main` / `layout-tree` / `layout-status` | `(layout-frontend l)` | 三块 `rect`（`layout-main` = `layout-frontend` 旧名） |
| `layout-rect` | `(layout-rect l id)` | 按 pane id 取 rect |
| `layout-pane-ids` / `layout-pane-visible?` | | 可见 pane 列表 / 单个显隐 |
| `layout-region-at` | `(layout-region-at l x y)` | 命中 `'frontend` / `'tree` / `'status` / `#f` |
| `layout-sidebar?` / `layout-sidebar-width` / `layout-status?` / `layout-status-height` | | 读口 |
| `layout-set-visible?` | `(layout-set-visible? l id on?)` | 按 pane id 显隐（纯几何） |
| `layout-set-sidebar?` / `layout-set-sidebar-width` / `layout-set-status?` / `layout-set-status-height` | | 写口（自动 normalize） |

### 6.2 panel（通用窗格协议）

```racket
(require "default-editor/panel.rkt")
```

`panel` 是 shell 与三个组件之间唯一的协议：id、自己的一份状态、四个回调。
shell 不需要认识「前端 / 树 / 状态栏」这些具体类型。

| 名字 | 签名 | 语义 |
|---|---|---|
| `panel-open` | `(panel-open id state #:visible? #t #:project #:resize #:refresh #:sync)` | 造窗格 |
| `panel-id` / `panel-visible?` / `panel-state` | | 读口 |
| `panel-set-state` / `panel-set-visible?` | | 写口（纯元数据） |
| `panel-project` | `(panel-project ed p)` | 投影成 `screen` |
| `panel-resize` | `(panel-resize ed p rect)` | 按矩形改尺寸，返回 `(values ed panel)` |
| `panel-refresh` | `(panel-refresh ed p)` | 全量刷新（可能重扫外部资源） |
| `panel-sync` | `(panel-sync ed p)` | 廉价同步（无 `sync` 回调时退化为 refresh） |

### 6.3 buffer（打开的文档注册表）

```racket
(require "default-editor/buffer.rkt")
```

buffer 是**独立值**，只依赖 core：它只记「用户在编辑的文档」，
不认识 tree / status / frontend / shell。每个 entry 有自己的 view，切换文档不丢视口状态。

| 名字 | 签名 | 语义 |
|---|---|---|
| `buffer-entry` / `buffer-entry-did` / `buffer-entry-vid` / `buffer-entry-path` / `buffer-entry-name` / `buffer-entry-saved-tick` | | 一条打开文档记录 |
| `buffers-empty` / `buffers?` / `buffers-count` / `buffers-entries` / `buffers-has?` | | 读口 |
| `buffers-active` / `buffers-active-entry` / `buffers-entry` / `buffers-find-path` | | 当前项 / 查找 |
| `buffers-add` | `(buffers-add ed bs text h w #:path #f #:name "*scratch*" #:history? #t #:line-numbers? #f)` | 新开文档（返回 `ed` / `bs` / `did`） |
| `buffers-remove` | `(buffers-remove bs did)` | 纯摘除，当前项落到邻居（不关 editor 里的 document） |
| `buffers-activate` / `buffers-next` / `buffers-prev` | | 切换 / 轮换 |
| `buffers-dirty?` / `buffers-mark-saved` / `buffers-rename` / `buffers-set-path` | | 脏标记 / 元数据 |

### 6.4 frontend（主编辑窗格）

```racket
(require "default-editor/frontend.rkt")
```

前端只管「把当前文档的一个 view 编辑好、投影好」。开新文件**不再关掉旧文档** ——
旧文档留在 buffer 里，可随时切回。

| 名字 | 签名 | 语义 |
|---|---|---|
| `frontend-create` | `(frontend-create text h w #:path #f #:name #f #:history? #t #:line-numbers? #f #:provider default-provider)` | 引导：建 editor + 第一个 buffer |
| `frontend-add` | `(frontend-add ed fe text #:path #f #:name #:height #f #:width #f)` | 追加文档并切过去（不关旧文档） |
| `frontend-open-file` | `(frontend-open-file ed fe path)` | 读盘打开；已打开则切过去 |
| `frontend-close` | `(frontend-close ed fe [did])` | **显式关文档**（buffer + editor 一起关；关完最后一个补 scratch） |
| `frontend-switch` / `frontend-next` / `frontend-prev` | | 切换 / 轮换 |
| `frontend-save` | `(frontend-save ed fe [path])` | 写盘 + 标干净 |
| `frontend-text` / `frontend-key` / `frontend-undo` / `frontend-redo` / `frontend-toggle-line-numbers` / `frontend-collapse` | | 编辑命令（键盘策略在这里） |
| `frontend-project` / `frontend-resize` / `frontend-panel` | | 投影 / 尺寸 / 转成 panel |
| `frontend-active-did` / `frontend-active-vid` / `frontend-path` / `frontend-name` / `frontend-dirty?` / `frontend-string` / `frontend-buffers` | | 读口 |
| `default-provider` | | 默认 face 策略（`'face` 属性 + `read-only`） |
...
| `tree-provider` / `tree-screen` / `tree-panel` | | 投影 / 转成 panel |
...
| `tree-close` | `(tree-close ed t)` | 显式关文件树文档 |
...
shell 刻意保持薄：状态是 `editor + layout + focus + panels`；组件调度全走 panel 协议，
输入路由与跨组件动作（树里回车 → 前端打开文件）全在这一层的命令里。

| 名字 | 签名 | 语义 |
|---|---|---|
| `shell-open` | `(shell-open [path #f] [rows 24] [cols 80] #:root (current-directory) #:sidebar? #t #:sidebar-width 30 #:status? #t #:status-height 1)` | 装配前端 + 树 + 状态栏 + 布局 |
| `shell-refresh` | `(shell-refresh s)` | 全量：重算几何 → 定尺寸 → 刷新每个 pane（树重扫） |
| `shell-sync` | `(shell-sync s)` | 廉价：只同步派生内容（状态栏） |
| `shell-resize` | `(shell-resize s rows cols)` | 改屏尺寸 |
| `shell-set-visible?` / `shell-toggle-pane` / `shell-toggle-sidebar` / `shell-set-sidebar?` / `shell-set-status?` / `shell-set-sidebar-width` / `shell-set-status-height` | | **窗口显隐**（不动文档） |
| `shell-open-file` / `shell-open-text` | `(shell-open-file s path)` | 开新文档进 buffer（旧文档保留） |
| `shell-close-buffer` | `(shell-close-buffer s [did])` | **显式关文档**（当前项切邻居） |
| `shell-switch-buffer` / `shell-next-buffer` / `shell-prev-buffer` | | 切换 / 轮换 buffer |
| `shell-write-file` | `(shell-write-file s [path])` | 写盘，返回 `(values shell ok?)` |
| `shell-focus` / `shell-focus-at` / `shell-toggle-focus` | `(shell-focus-at s 'tree)` | 焦点（隐藏的 pane 拒绝聚焦） |
| `shell-set-message` / `shell-clear-message` | | 状态栏消息 |
| `shell->string` / `shell-active-did` / `shell-main-did` / `shell-main-vid` / `shell-path` / `shell-name` / `shell-message` / `shell-sidebar?` / `shell-status?` / `shell-buffers` | | 读口 |
| `shell->screen` | `(shell->screen s)` | 把可见 pane `screen-compose` 拼屏，active = 焦点 |
...
| `shell-editor` / `shell-tree` / `shell-status` / `shell-frontend` / `shell-layout` / `shell-panels` / `shell-panel` | | 读口（组件状态 / 几何 / 窗格） |

默认键位：`←→↑↓ Home End PgUp PgDn` 导航；`Shift+方向` 扩选；`Alt+↑↓` 加光标；
`^Z ^Y` 撤销/重做；`^S` 保存；`^X` 关当前文档；`^W` 切焦点；`^B` 开关侧栏；
`Ctrl+PgDn/PgUp` 下一个/上一个文档；`^Q` 退出；`Esc` 收敛选区/退出；
