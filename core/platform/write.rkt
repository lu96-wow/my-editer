...
;;;
;;;   editor-swap-document   换某 document-entry 的 document 值（entries + 同文档 view 的引用）
...
;; 换某 document-entry 的 document 值：entries + 同文档 view 的 window.document 一起换。
;; **不**动光标、不滚屏、不 ensure（即使文本变了，光标也不映射）。
(define (editor-swap-document ed did d*)
  (define d0 (document-entry-document (editor-document-entry ed did)))
  (struct-copy editor ed
    [documents (for/list ([e (in-list (editor-documents ed))])
               (if (= (document-entry-id e) did) (struct-copy document-entry e [document d*]) e))]
    [views (for/list ([v (in-list (editor-views ed))])
...
(define (editor-set-view-document ed vid did)
...
(define (editor-record-history ed did replay undo pre-point)
  (define entry (editor-document-entry ed did))
  (define entry* (struct-copy document-entry entry
                  [history (history-record (document-entry-history entry) replay undo pre-point)]))
  (struct-copy editor ed
    [documents (for/list ([e (in-list (editor-documents ed))])
               (if (= (document-entry-id e) did) entry* e))]))

(define (editor-put-document-name ed did name)
  (struct-copy editor ed
    [documents (for/list ([e (in-list (editor-documents ed))])
               (if (= (document-entry-id e) did) (struct-copy document-entry e [name name]) e))]))

(define (editor-put-history ed did h)
  (struct-copy editor ed
    [documents (for/list ([e (in-list (editor-documents ed))])
...
(module+ test
  ...
  (check-eq? (window-document (view-window (editor-view-ref e1 0)))
             (document-entry-document (editor-document-entry e1 0)))
  ...
  (check-equal? (document->string (document-entry-document (editor-document-entry e2 0))) "XYold")
