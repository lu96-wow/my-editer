...
;;; 这里只有数据与读取。
;;;
;;; 每个 buffer-entry 持有一个 **document**（纯文本 buffer ⊕ 标注 attrs）；
;;; view 看哪个文档由它的 `window.document` 决定。引用完整性（任一 view 的
;;; window.document 必是某个 buffer-entry 的 document）由 write.rkt 维持。
...
(struct buffer-entry (id name document history) #:transparent)
...
(define (document-id-of ed d)
  (define e (for/first ([e (in-list (editor-buffers ed))]
                        #:when (eq? d (buffer-entry-document e)))
              e))
...
(module+ test
  ...
  (define ed (editor (list (buffer-entry 0 "s" d 'H))
