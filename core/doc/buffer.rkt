...
(require "../atom/point.rkt" "../atom/content.rkt" "../atom/edit.rkt"
         ...
         "../unit/restrictions.rkt" rackunit)

;;; doc/buffer.rkt —— 文档：文本 + 作者态约束（无光标、无 face）
;;;
...
;;;
;;; 文档**只存作者态**：content + restrict（read-only 等）。
;;; **face 不进文档**：派生 face 走投影时的 face-provider（viewport/render.rkt）。
;;;
...
(struct buffer
  (content      ; content.rkt
   restrictions ; restrictions（作者态约束）
   tick)        ; nat      任何改动 +1（编辑 / 写约束）
  #:transparent)
...
;; 内容是否同一：只有 splice 才换新 content（不可变 struct），写约束不改 content。
;; 这是「用户改了内容」与「只写了约束」的精确区分。
...
;; 某点的约束。读口；要只要「是否只读」用 (restrict-read-only? (buffer-restrict-at …))。
(define (buffer-restrict-at b p)
  (restrictions-at (buffer-restrictions b) (point-line p) (point-col p)))

;; 一行内约束段：(listof (list start end restrict))，恰好覆盖整行。
(define (buffer-restrict-runs b line)
  (restrictions-runs (buffer-restrictions b) line
...
(define (buffer-apply-edit* b d guard?)
  ...
  (cond
    ...
    [else
     (define rs* (restrictions-apply-edit (buffer-restrictions b) d*))
     (values (buffer content* rs* (add1 (buffer-tick b)))
             d*)]))
